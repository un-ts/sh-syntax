 ...
 *
 * - zsh parameter-expansion word modifiers such as `:A` (absolute path) and
 * `:h` (dirname) are valid zsh but not valid bash.
 *
 * - In bash mode, `mvdan/sh` reads `${p:offset:length}` as a substring
 *   expansion whose `length` is an arithmetic expression, so a third `:` yields
 *   `ternary operator missing ? before :`.
 *
